...
scripts/
  ...
  prove_release_gate_context.sh # release-gate runbook-context backfill/fallback proof runner
  ...
  prove_policy_tamper.sh  # G29 policy tamper detection proof
  chaos/network_partition.sh # G4.2 kafka network partition drill (disconnect/pause + recovery)
...
runbooks/
  ...
  mapping_coverage_failure.sh # mapping coverage failure automated drill
...
make chaos-network-partition # broker network isolation drill
```
...
- `chaos_network_partition_ok=true`
- `chaos_network_partition_report=build/chaos/network-partition-<timestamp>.json`
- `chaos_network_partition_latest=build/chaos/network-partition-latest.json`
- `chaos_network_partition_during_reachable=false`
...
# include policy-tamper runbook in same gate:
./scripts/verification_factory.sh --run-policy-tamper
# include kafka network-partition runbook in same gate:
...
- summary includes `run_load_profiles=true|false`, `run_startup_guardrails=true|false`, `run_change_workflow=true|false`, `run_policy_signature=true|false`, `run_policy_tamper=true|false`, `run_network_partition=true|false`, `run_redpanda_bounce=true|false`, `run_exactly_once_runbook=true|false`, `run_mapping_integrity_runbook=true|false`, `run_mapping_coverage_runbook=true|false`, `run_idempotency_latch_runbook=true|false`, `run_idempotency_key_format_runbook=true|false`, `run_proof_health_runbook=true|false`, `run_determinism=true|false`, `run_exactly_once_million=true|false`, `run_adversarial=true|false`, `compliance_require_full_mapping=true|false` and optional artifacts (`load_all_report`, `startup_guardrails_runbook_dir`, `change_workflow_runbook_dir`, `policy_signature_runbook_dir`, `policy_signature_runbook_summary`, `policy_signature_runbook_summary_latest`, `policy_tamper_runbook_dir`, `policy_tamper_runbook_summary`, `policy_tamper_runbook_summary_latest`, `network_partition_runbook_dir`, `network_partition_runbook_summary`, `network_partition_runbook_summary_latest`, `redpanda_bounce_runbook_dir`, `redpanda_bounce_runbook_summary`, `redpanda_bounce_runbook_summary_latest`, `exactly_once_runbook_dir`, `exactly_once_runbook_summary`, `exactly_once_runbook_summary_latest`, `mapping_integrity_runbook_dir`, `mapping_integrity_runbook_summary`, `mapping_integrity_runbook_summary_latest`, `mapping_coverage_runbook_dir`, `mapping_coverage_runbook_summary`, `mapping_coverage_runbook_summary_latest`, `idempotency_latch_runbook_dir`, `idempotency_latch_runbook_summary`, `idempotency_latch_runbook_summary_latest`, `idempotency_key_format_runbook_dir`, `idempotency_key_format_runbook_summary`, `idempotency_key_format_runbook_summary_latest`, `proof_health_runbook_dir`, `proof_health_runbook_summary`, `proof_health_runbook_summary_latest`, `adversarial_runbook_dir`, `adversarial_runbook_summary`, `adversarial_runbook_summary_latest`, `policy_smoke_report`, `prove_policy_tamper_report`, `prove_determinism_report`, `prove_idempotency_report`, `prove_idempotency_key_format_report`, `prove_latch_approval_report`, `prove_exactly_once_million_report`, `prove_mapping_integrity_report`, `proof_health_metrics_report`, `adversarial_tests_report`, `budget_failure_runbook_dir`, `verify_change_audit_chain_report`, `prove_controls_freshness_report`, `prove_budget_freshness_report`, `prove_release_gate_context_report`, `anomaly_detector_report`)
...
### 15.1) Policy tamper proof
```bash
make prove-policy-tamper
```
Success output includes:
- `prove_policy_tamper_report=build/policy/prove-policy-tamper-<timestamp>.json`
- `prove_policy_tamper_latest=build/policy/prove-policy-tamper-latest.json`
...
- when reports exist, budget checks include `auditChain`, `changeAuditChain`, `piiLogScan`, `anomaly`, `complianceEvidence`, `releaseGateContext`, `releaseGateContextProof`, `mappingIntegrity`, `mappingCoverage`, `mappingIntegrityRunbook`, `mappingCoverageRunbook`, `proofHealth`, `idempotencyScope`, `idempotencyKeyFormat`, `latchApproval`, `idempotencyLatchRunbook`, `idempotencyKeyFormatRunbook`, `proofHealthRunbook`, `exactlyOnceMillion` gates
...
- `releaseGateContextProof` check supports policy fields:
  - `mustBeOk`
  - `maxFailedChecks`
  - `mustExpectRequireRunbookContext`
...
### 16.4) Idempotency+latch failure runbook
```bash
make runbook-idempotency-latch
# allow drill to continue while collecting evidence:
RUNBOOK_ALLOW_PROOF_FAIL=true make runbook-idempotency-latch
```
Outputs:
...
```bash
make prove-mapping-integrity
```
Outputs:
- `prove_mapping_integrity_report=build/compliance/prove-mapping-integrity-<timestamp>.json`
- `prove_mapping_integrity_latest=build/compliance/prove-mapping-integrity-latest.json`
...
### 19.3) Mapping coverage failure runbook
```bash
make runbook-mapping-coverage
```
Outputs:
...
### 19.4) Release-gate context proof
```bash
make prove-release-gate-context
# diagnostic mode (do not fail when release/fallback artifacts are absent):
./scripts/prove_release_gate_context.sh --allow-missing
```
Outputs:
- `prove_release_gate_context_report=build/release-gate/prove-release-gate-context-<timestamp>.json`
- `prove_release_gate_context_latest=build/release-gate/prove-release-gate-context-latest.json`
...
```bash
make runbook-policy-tamper
# allow drill to continue even if tamper proof fails:
RUNBOOK_ALLOW_POLICY_TAMPER_FAIL=true make runbook-policy-tamper
# allow budget fail while only diagnosing tamper path:
RUNBOOK_ALLOW_BUDGET_FAIL=true make runbook-policy-tamper
```
Outputs:
...
```bash
make runbook-network-partition
# allow drill to continue even if chaos partition check fails:
RUNBOOK_ALLOW_NETWORK_PARTITION_FAIL=true make runbook-network-partition
# allow budget fail while only diagnosing partition path:
RUNBOOK_ALLOW_BUDGET_FAIL=true make runbook-network-partition
```
Outputs:
...
make runbook-mapping-coverage
# allow drill to continue while collecting evidence:
RUNBOOK_ALLOW_PROOF_FAIL=true make runbook-mapping-coverage
```
Outputs:
- `runbook_mapping_coverage_ok=true|false`
- `runbook_budget_ok=true|false`
...
make runbook-proof-health
# allow drill to continue while collecting evidence:
RUNBOOK_ALLOW_PROOF_FAIL=true make runbook-proof-health
```
Outputs:
...
# include policy-tamper runbook in gate:
...
# include mapping-coverage failure runbook in gate:
...
- report includes exactly-once runbook context: `exactly_once_runbook_ok`, `exactly_once_runbook_budget_ok`, `exactly_once_runbook_proof_ok`, `exactly_once_runbook_proof_repeats`, `exactly_once_runbook_recommended_action`
- report includes mapping-integrity runbook context: `mapping_integrity_runbook_ok`, `mapping_integrity_runbook_budget_ok`, `mapping_integrity_runbook_proof_ok`, `mapping_integrity_runbook_recommended_action`
- report includes mapping-coverage runbook context: `mapping_coverage_runbook_ok`, `mapping_coverage_runbook_budget_ok`, `mapping_coverage_runbook_proof_ok`, `mapping_coverage_runbook_recommended_action`
