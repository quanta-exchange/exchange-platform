...
LATEST_SUMMARY_FILE="$ROOT_DIR/build/runbooks/policy-signature-latest.json"
...

{
  ...
  python3 - "$SUMMARY_FILE" "$POLICY_REPORT" "$POLICY_CODE" "$BUDGET_OK" "$RUNBOOK_ALLOW_POLICY_FAIL" "$RUNBOOK_ALLOW_BUDGET_FAIL" <<'PY'
import json
import pathlib
import sys
...
allow_policy_fail = sys.argv[5].lower() == "true"
...
runbook_ok = (
    ((policy_exit_code == 0) and policy_ok) or allow_policy_fail
) and (budget_ok or allow_budget_fail)

summary = {
    "generated_at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
    "runbook_ok": runbook_ok,
    "allow_policy_fail": allow_policy_fail,
...
PY
  ...
  echo "policy_signature_summary_file=$SUMMARY_FILE"
  echo "policy_signature_summary_latest=$LATEST_SUMMARY_FILE"
  echo "runbook_policy_signature_ok=$SUMMARY_RUNBOOK_OK"
