...

{
  ...
  python3 - "$SUMMARY_FILE" "$CHAOS_REPORT" "$CHAOS_CODE" "$BUDGET_OK" "$DURING_REACHABLE" "$APPLIED_METHOD" <<'PY'
import json
import pathlib
import sys
...
PY
  ...
  RUNBOOK_OK=true
  if [[ "$CHAOS_CODE" -ne 0 && "$RUNBOOK_ALLOW_NETWORK_PARTITION_FAIL" != "true" ]]; then
    RUNBOOK_OK=false
  fi
  ...
  echo "runbook_network_partition_ok=$RUNBOOK_OK"
