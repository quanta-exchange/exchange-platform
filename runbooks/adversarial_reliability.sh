...

{
  ...
  python3 - "$SUMMARY_FILE" "$ADVERSARIAL_REPORT" "$ADVERSARIAL_CODE" "$BUDGET_OK" <<'PY'
import json
import pathlib
import sys
...
PY
  ...
  RUNBOOK_OK=true
  if [[ "$ADVERSARIAL_CODE" -ne 0 && "$RUNBOOK_ALLOW_ADVERSARIAL_FAIL" != "true" ]]; then
    RUNBOOK_OK=false
  fi
  ...
  echo "runbook_adversarial_reliability_ok=$RUNBOOK_OK"
