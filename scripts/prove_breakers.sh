...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/build/breakers}"
TEST_FILTER="${TEST_FILTER:-risk::tests::order_}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

mkdir -p "$OUT_DIR"
TS_ID="$(date -u +"%Y%m%dT%H%M%SZ")"
LOG_FILE="$OUT_DIR/prove-breakers-$TS_ID.log"
REPORT_FILE="$OUT_DIR/prove-breakers-$TS_ID.json"
LATEST_FILE="$OUT_DIR/prove-breakers-latest.json"
...
echo "prove_breakers_report=$REPORT_FILE"
echo "prove_breakers_latest=$LATEST_FILE"
echo "prove_breakers_ok=$OK"
